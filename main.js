...
function findInvalidCards(cards, cb) {
  ...
  for (let card in cards) {
      cb(cards[cards])?console.log('Valid'):invalidCards.push(cards[card])
  }
  return invalidCards
...
function idInvalidCardCompanies(invalidCards) {
  ...
  for(let card in invalidCards) {
    for (company in companies) {
      if (invalidCards[card] === companies[company]) {
        if (!invalidCardsSenders.includes(company)){
          invalidCardsSenders.push(company)
        }
      }
    }
  }
